...
export default function Page() {
  ...
  const pie = portfolio.map(p => ({ name: p.name, value: Math.round(p.value/total*100) }))
  return (
      ...
      <Card>
        <CardContent>
          ...
          <div className='h-72'>
            <ResponsiveContainer width='100%' height='100%'>
              <PieChart>
                <Pie data={pie} dataKey='value' nameKey='name' outerRadius={120} label>
                  {pie.map((entry, index) => <Cell key={index} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
